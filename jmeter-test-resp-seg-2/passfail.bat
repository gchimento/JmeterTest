...
@if not defined REQ (
	...
	@echo     No se llego a la cantidad especificada de requests (%HITS%^): hits^<%HITS% over 90s, stop as failed >>passfail.yaml
	...
	@echo     No se llego a la cantidad especificada de requests (%HITS%^): hits of %REQ%^<%HITS% over 90s, stop as failed >>passfail.yaml
	echo Se setea el porcentaje maximo de error en %PERERR%%% para el Total
	@echo     Se supero el procentaje maximo de errores (%PERERR%%%^): fail^>%PERERR%%%, continue as failed >>passfail.yaml
)